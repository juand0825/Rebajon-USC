...
class ReporteDao {
  Future<ReporteResumenModel> obtenerResumen() async {
    ...
    return ReporteResumenModel(
      totalVentas: int.parse(
        ventas.rows.first.assoc()['total_ventas'].toString(),
      ),
      totalIngresos: double.parse(
        ventas.rows.first.assoc()['total_ingresos'].toString(),
      ),
      totalProductosVendidos: int.parse(
        productosVendidos.rows.first.assoc()['total_productos'].toString(),
      ),
      alertasPendientes: int.parse(
        alertasPendientes.rows.first.assoc()['total'].toString(),
      ),
      alertasResueltas: int.parse(
        alertasResueltas.rows.first.assoc()['total'].toString(),
      ),
      medicamentosSinStock: int.parse(
        sinStock.rows.first.assoc()['total'].toString(),
      ),
      medicamentosStockMinimo: int.parse(
        stockMinimo.rows.first.assoc()['total'].toString(),
      ),
      lotesRegistrados: int.parse(lotes.rows.first.assoc()['total'].toString()),
    );
  }

  Future<List<ReporteVentaProductoModel>> productosMasVendidos() async {
    final conn = await DbConnection.getConnection();
